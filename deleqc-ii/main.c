...
void imprimirIndividuo( double **populacao, int coordenadas, int id, int geracao, int restricoes, int nRep, int numberOfValuesInEachIndividual, int nGs, int nHs, int print );
...
int main( int argc, char** argv ) {
	...
	double epsilonCorrecao = 0.000001;
	int nReparos = 0;
...
//        getchar();
		...
		imprimirIndividuo(populacao, N_Coordenada, idMelhorIndividuo, 0, Restricoes, nReparos, numberOfValuesInEachIndividual, nGs, nHs, PRINT_ALL);
        ...
        //printf("N_Individuo: %d\n", N_Individuo);
            ...
            for (i = 0; i < N_Individuo; ++i) {
                ...
                Avalia(novos_ind[i], N_Coordenada, prob, Matriz_Coef, bounds, epsilonCorrecao, x_star);
                //CEC2018
				...
				if ( individuoViolaRestricaoNormaInfinito( populacao, N_Coordenada, idMelhorIndividuo, g, Restricoes, epsilonCorrecao ) == 0 ) {
					...
					if ( g < N_Geracoes ) {
						...
						for (i = 0; i < N_Individuo; ++i) {
							...
							Avalia(populacao[i], N_Coordenada, prob, Matriz_Coef, bounds, epsilonCorrecao, x_star);
							//CEC2018
							//evaluate the bound and linear violations: APM and DEb's TS
							if (constraintHandler != DELEQC_ONLY) {
								boundConstraints(populacao[i], N_Coordenada, nGs, nHs, bounds);
						...
						imprimirIndividuo(populacao, N_Coordenada, idMelhorIndividuo, geracoes, Restricoes, nReparos, numberOfValuesInEachIndividual, nGs, nHs, PRINT_ALL);
					}
    ...
    imprimirIndividuo(populacao, N_Coordenada, idMelhorIndividuo, geracoes, Restricoes, nReparos, numberOfValuesInEachIndividual, nGs, nHs, 1);
    ...
    double time_spent = (1000.0*(end - begin - printT)) / CLOCKS_PER_SEC;
    printf("time (ms): %lf\n", time_spent);
    ///printf("clock/s: %ld\n", CLOCKS_PER_SEC);
...
double** Inicializa_Populacao(int Restricoes, int N_Coordenada, int N_Individuo, int prob, double** bounds, int nGs, int nHs, int* sizeOfIndividual){
    ...
    switch(prob){
        ...
        case 25: 
            ...
            break;  
        case 26: 
            arq = fopen("input_data/Entrada26.txt", "r");
            if (arq == NULL){
            printf("Problemas na LEITURA do arquivo\n");
            }
            break;  
        case 27: 
            arq = fopen("input_data/Entrada27.txt", "r");
            if (arq == NULL){
            printf("Problemas na LEITURA do arquivo\n");
            }
            break;  
        case 34: 
            arq = fopen("input_data/Entrada34.txt", "r");
            if (arq == NULL){
            printf("Problemas na LEITURA do arquivo\n");
            }
            break;  
        case 35: 
            arq = fopen("input_data/Entrada35.txt", "r");
            if (arq == NULL){
            printf("Problemas na LEITURA do arquivo\n");
            }
            break;  
...
*/
    ...
    */
	...
	for(i = 0; i < N_Individuo; i++) {
		...
		//int nb =  (*sizeOfIndividual) * sizeof(double);
		//printf("nb: %d\n", nb);
		//printf("i: %d\n", i);
		populacao[i] = malloc( (*sizeOfIndividual) * sizeof(double));;
...
void Avalia_Populacao(double **populacao, int N_Individuo, int N_Coordenada, int prob, double **bounds, double epsilon){
	...
	for(i = 0; i < N_Individuo; i++){
        ...
        Avalia(populacao[i], N_Coordenada, prob, Matriz_Coef, bounds, epsilon, x_star);
	}
}
...
//	scanf("%d",&(*prob));
     ...
     if(argc >= 9) {
        ...
        *epsilonCorrecao = atof(argv[8]);
        ...
        if(argc == 11 && ( *problema_numero <= 100 && !(*problema_numero >= 20 && *problema_numero <= 27) && *problema_numero != 30 ) ) { // UTILIZADOS APENAS NOS PROBLEMAS 7 a 11
...
void printPopulation( double **populacao, int coordenadas, int id, int geracao, int restricoes, int nReparos, int numberOfValuesInEachIndividual, int nGs, int nHs, int populationSize ) {
	...
	for(int i=0; i<populationSize; i++) {
		imprimirIndividuo( populacao, coordenadas, i, geracao, restricoes, nReparos, numberOfValuesInEachIndividual, nGs, nHs, PRINT_ALL);
	}
	printf("------------------------------------\n\n");

}

void imprimirIndividuo( double **populacao, int coordenadas, int id, int geracao, int restricoes, int nReparos, int numberOfValuesInEachIndividual, int nGs, int nHs, int print ) {
